...
上一篇文档[「JuiceFS 单机模式快速上手指南」](./standalone.md)通过采用「对象存储」和 BadgerDB 的组合，创建了一个单机文件系统。BadgerDB 是本地元数据存储，更适合单个 JuiceFS 进程使用。

如果要让一个文件系统可以在分布式环境中被多个计算机同时挂载读写，需要使用支持通过网络访问的元数据引擎，例如 Redis 兼容数据库。

本文以上一篇文档为基础，进一步将本地 BadgerDB 替换成多用户的「云数据库」，从而实现可以在网络上任何一台计算机上进行挂载读写的分布式文件系统。
...
1. **单机数据库**：通常只能单机访问，如 BadgerDB；
2. **基于网络的数据库**：提供基于网络的访问接口，支持多用户同时访问，如 Redis 兼容数据库。

JuiceFS 目前支持 Redis 兼容数据库作为基于网络的元数据存储。

不同数据库的性能和稳定性表现各不相同。Redis 是内存型键值数据库，性能极为出色，但可靠性取决于部署方式和持久化配置。

有关数据库选择方面的内容，我们会专门编写文档进行介绍。

## 云数据库

云计算平台通常提供 Redis 兼容的内存型数据库产品，例如 Amazon ElastiCache。经过简单的初始化设置就可以创建出多副本、高可用的数据库集群。
