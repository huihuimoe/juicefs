...
|`--cache-index=memory`|缓存索引模式（`memory` 或 `path`）。`path` 通过确定性路径直接读取缓存文件来降低内存占用，并会禁用周期性缓存扫描和缓存过期清理；默认对 `cache-size` / `cache-items` 使用尽力式采样淘汰，但设置 `--cache-eviction=lru` 时会维护一个内存 LRU 淘汰索引。|
