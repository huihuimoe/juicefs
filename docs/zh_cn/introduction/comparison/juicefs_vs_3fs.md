...
- **元数据引擎**：用于存储文件元数据，包括常规文件系统的元数据和文件数据的索引。社区版支持 Redis 兼容服务和 BadgerDB。企业版使用自研高性能元数据服务。
...
JuiceFS 社区版的元数据模块提供一组操作元数据的接口，可以接入 Redis 兼容服务和 BadgerDB。JuiceFS 企业版使用自研高性能元数据服务，可根据负载情况来平衡数据和热点操作，以避免大规模训练中元数据服务热点集中在某些节点的问题。
