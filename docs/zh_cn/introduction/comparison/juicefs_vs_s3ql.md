...
- S3QL 仅支持 SQLite 一种数据库，而 JuiceFS 支持 Redis 兼容服务和 BadgerDB。
...
| :-------------------- | :----------------- | :------------------------- |
| 项目状态              | 活跃维护              | 活跃开发                    |
| 元数据引擎            | SQLite             | Redis 兼容服务、BadgerDB |
...
S3QL 和 JuiceFS 都使用数据库保存元数据，S3QL 仅支持 SQLite 数据库，JuiceFS 支持 Redis 兼容服务和 BadgerDB。
...
```shell
sudo juicefs mount -d badger://myjfs.db mnt-juicefs
```
...
**JuiceFS** 支持对象存储、HDFS、WebDAV、本地磁盘作为数据存储引擎，支持 Redis 兼容服务和 BadgerDB 作为元数据存储引擎。除了通过 FUSE 提供标准的 POSIX 文件系统接口以外，JuiceFS 还提供 Java API，可以直接替代 HDFS 为 Hadoop 提供存储。同时还提供 [Kubernetes CSI Driver](https://github.com/juicedata/juicefs-csi-driver)，可以作为 Kubernetes 的存储层做数据持久化存储。JuiceFS 是为企业级分布式数据存储场景设计的文件系统，广泛应用于大数据分析、机器学习、容器共享存储、数据共享及备份等多种场景。
