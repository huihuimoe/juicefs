...
| :--- | :--- | :--- |
| 元数据引擎 | 支持多种数据库 | 社区版支持 Redis 兼容服务和 BadgerDB，企业版使用自研高性能元数据引擎 |
...
- 元数据被保存在用户自行选择的元数据引擎中，如 Redis 兼容服务或 BadgerDB。
...
- JuiceFS 对数据库事务能力要求更高（下方会详细介绍），当前支持 [Redis 兼容服务和 BadgerDB](../../reference/how_to_set_up_metadata_engine.md)。

### 原子性操作

* JuiceFS 严格确保每一项操作的原子性，因此对于元数据引擎的事务能力有着较强的要求，支持的数据库更少。
...
JuiceFS 社区版没有实现变更日志，但可以自行使用元数据引擎和对象存储自身的数据复制能力实现文件系统镜像功能。比方说 [Redis 兼容复制](https://redis.io/docs/management/replication) 配合 [S3 的复制对象功能](https://docs.aws.amazon.com/zh_cn/AmazonS3/latest/userguide/replication.html)，就能脱离 JuiceFS 实现类似 SeaweedFS 的 Active-Passive 模式。
