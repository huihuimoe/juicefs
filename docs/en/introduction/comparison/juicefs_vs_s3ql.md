...
- S3QL only supports SQLite. JuiceFS supports Redis-compatible services and BadgerDB.
...
| Metadata engine           | SQLite                | Redis-compatible, BadgerDB     |
...
Both S3QL and JuiceFS use a database to store metadata. S3QL only supports SQLite databases, while JuiceFS supports Redis-compatible services and BadgerDB.
