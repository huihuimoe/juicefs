...
|`--cache-index=memory`|cache index mode (`memory` or `path`). `path` reduces memory usage by reading cache files directly from deterministic paths, disables in-memory cache scans and cache expiration, and uses best-effort sampled eviction for `cache-size` / `cache-items` limits.|
